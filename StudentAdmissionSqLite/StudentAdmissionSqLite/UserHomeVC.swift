...
class UserHomeVC: UIViewController {
    ...
    public let mybtn1 : UIButton = {
        let btn = UIButton()
         btn.setImage(UIImage(named: "not"), for: .normal)
    ...
    public let mybtn2 : UIButton = {
        let btn = UIButton()
        btn.setImage(UIImage(named: "password"), for: .normal)
        btn.backgroundColor = .white
        btn.addTarget(self, action: #selector(logout), for: .touchUpInside)
        btn.layer.cornerRadius = 30
        return btn
    ...
    public let mybtn3 : UIButton = {
        let btn = UIButton()
         btn.setImage(UIImage(named: "userlogin"), for: .normal)
        btn.backgroundColor = .white
        btn.addTarget(self, action: #selector(logout), for: .touchUpInside)
        btn.layer.cornerRadius = 30
        return btn
    ...
    override func viewDidLayoutSubviews() {
        ...
        label1.frame = CGRect(x:view.width / 2 - 100   , y:myimg.bottom + 10 , width: 200, height: 40)
